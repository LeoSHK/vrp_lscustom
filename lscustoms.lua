...
vRPLSc = {}
Tunnel.bindInterface("vrp_lscustom",vRPLSc)
Proxy.addInterface("vrp_lscustom",vRPLSc)
LSserver = Tunnel.getInterface("vrp_lscustom")
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("Seu veiculo foi Reparado.")
		else
			LSCMenu:showNotification("Item foi Comprado.")
		end
		return true
	else
		LSCMenu:showNotification("~r~Você não pode comprar este item.")
		return false
	end
end
